...
#include <time.h> 
#define input_file "KA.AN2.40.jpg" //Performing convolution on one image
using namespace std;
using namespace cv;
#define h 224
#define w 224
...
string layer0 = "model_weights/block1_conv1.txt";
string layer1 = "model_weights/block1_conv2.txt";
string layer2 = "model_weights/block2_conv1.txt";
string layer3 = "model_weights/block2_conv2.txt";
string layer4 = "model_weights/block3_conv1.txt";
string layer5 = "model_weights/block3_conv2.txt";
string layer6 = "model_weights/block3_conv3.txt";
string layer7 = "model_weights/block3_conv4.txt";
string layer8 = "model_weights/block4_conv1.txt";
string layer9 = "model_weights/block4_conv2.txt";
string layer10 = "model_weights/block4_conv3.txt";
string layer11 = "model_weights/block4_conv4.txt";
string layer12 = "model_weights/block5_conv1.txt";
string layer13 = "model_weights/block5_conv2.txt";
string layer14 = "model_weights/block5_conv3.txt";
string layer15 = "model_weights/block5_conv4.txt";
string fclayer0 = "dense.txt";
string fclayer1 = "dense1.txt";


//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
///Kernel function to convert from image to column
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
__global__ void im_to_col(int* r, int* g, int* b, double* result, int cols)
{
...
__global__ void input2col(double* input, int input_size, double* result, int input_channels, int output_size, int stride) 
{
	int k;
	int kernel_size = 3;
	int idx = blockDim.x * blockIdx.x + threadIdx.x;
...
__global__ void input_pad(double* input, int input_size, double* output, int pad_size) 
{
...
int main()
{
	...
	double* dim = (double*)calloc(68, sizeof(double));
	readdim(dim);
	...
	cudaMalloc((void**)& gpu_im_to_col_l0, pow(output_size, 2)* pow(kernel_size, 2)* input_channels * sizeof(double));
	...
	//Call the im_to_col kernel function

	//copy from GPU to CPU

	im_to_col << < gridDim_l0, blockDim_l0 >> > (gpu_r, gpu_g, gpu_b, gpu_im_to_col_l0, input_size);
	cudaDeviceSynchronize();

	double* result = (double*)calloc(pow(output_size, 2) * pow(kernel_size, 2) * input_channels, sizeof(double));
	cudaMemcpy(result, gpu_im_to_col_l0, pow(output_size, 2)* pow(kernel_size, 2)* input_channels * sizeof(double), cudaMemcpyDeviceToHost);
	cudaDeviceSynchronize();
	...
	weight_mult << < gridDim_W0, blockDim_W0 >> > (gpu_W0, gpu_B0, gpu_im_to_col_l0, input_channels, output_size, gpu_output_l0);
	cudaDeviceSynchronize();
	...
	input_pad << < gridDim_l1, blockDim_l1 >> > (gpu_output_l0, input_size, gpu_input_l1, pad_size); 
	cudaDeviceSynchronize();
	cudaFree(gpu_output_l0);


	// convert to columns using im_to_col
	double* gpu_im_to_col_l1;
	cudaMalloc((void**)& gpu_im_to_col_l1, pow(output_size , 2) * pow(kernel_size, 2) * input_channels * sizeof(double));
	input_size = input_size + 2 * pad_size;
	dim3 gridDim_I1(1);
	dim3 blockDim_I1(output_size);
	input2col << < gridDim_I1, blockDim_I1 >> > (gpu_input_l1, input_size, gpu_im_to_col_l1, input_channels, output_size, stride);
	...
	weight_mult << < gridDim_W1, blockDim_W1 >> > (gpu_W1, gpu_B1, gpu_im_to_col_l1, input_channels, output_size, gpu_output_l1);
	free(W1);
	free(B1);
	cudaFree(gpu_input_l1);
	cudaFree(gpu_W1);
	cudaFree(gpu_B1);
	cudaFree(gpu_im_to_col_l1);
	...
	// convert to columns using im_to_col
	double* gpu_im_to_col_l2;
	cudaMalloc((void**)& gpu_im_to_col_l2, pow(output_size ,2) * pow(kernel_size, 2) * input_channels * sizeof(double));
	input_size = input_size + 2 * pad_size;
	dim3 gridDim_I2(1);
	dim3 blockDim_I2(output_size);
	input2col << < gridDim_I2, blockDim_I2 >> > (gpu_input_l2, input_size, gpu_im_to_col_l2, input_channels, output_size, stride);
	...
	weight_mult << < gridDim_W2, blockDim_W2 >> > (gpu_W2, gpu_B2, gpu_im_to_col_l2, input_channels, output_size, gpu_output_l2);
	free(W2);
	free(B2);

	cudaFree(gpu_W2);
	cudaFree(gpu_B2);
	cudaFree(gpu_im_to_col_l2);
	...
	// convert to columns using im_to_col
	double* gpu_im_to_col_l3;
	cudaMalloc((void**)& gpu_im_to_col_l3, pow(output_size,2) * pow(kernel_size, 2) * input_channels * sizeof(double));
	input_size = input_size + 2 * pad_size;
	dim3 gridDim_I3(1);
	dim3 blockDim_I3(output_size);
	input2col << < gridDim_I3, blockDim_I3 >> > (gpu_input_l3, input_size, gpu_im_to_col_l3, input_channels, output_size, stride);
	...
	weight_mult << < gridDim_W3, blockDim_W3 >> > (gpu_W3, gpu_B3, gpu_im_to_col_l3, input_channels, output_size, gpu_output_l3);
	free(W3);
	free(B3);

	cudaFree(gpu_W3);
	cudaFree(gpu_B3);
	cudaFree(gpu_im_to_col_l3);
	...
	// convert to columns using im_to_col
	double* gpu_im_to_col_l4;
	cudaMalloc((void**)& gpu_im_to_col_l4, pow(output_size , 2) * pow(kernel_size , 2) * input_channels * sizeof(double));
	input_size = input_size + 2 * pad_size;
	dim3 gridDim_I4(1);
	dim3 blockDim_I4(output_size);
	input2col << < gridDim_I4, blockDim_I4 >> > (gpu_input_l4, input_size, gpu_im_to_col_l4, input_channels, output_size, stride);
	...
	weight_mult << < gridDim_W4, blockDim_W4 >> > (gpu_W4, gpu_B4, gpu_im_to_col_l4, input_channels, output_size, gpu_output_l4);
	free(W4);
	free(B4);

	cudaFree(gpu_W4);
	cudaFree(gpu_B4);
	cudaFree(gpu_im_to_col_l4);
	...
	// convert to columns using im_to_col
	double* gpu_im_to_col_l5;
	cudaMalloc((void**)& gpu_im_to_col_l5, pow(output_size, 2) * pow(kernel_size , 2) * input_channels * sizeof(double));
	input_size = input_size + 2 * pad_size;
	dim3 gridDim_I5(1);
	dim3 blockDim_I5(output_size);
	input2col << < gridDim_I5, blockDim_I5 >> > (gpu_input_l5, input_size, gpu_im_to_col_l5, input_channels, output_size, stride);
	...
	weight_mult << < gridDim_W5, blockDim_W5 >> > (gpu_W5, gpu_B5, gpu_im_to_col_l5, input_channels, output_size, gpu_output_l5);
	free(W5);
	free(B5);

	cudaFree(gpu_W5);
	cudaFree(gpu_B5);
	cudaFree(gpu_im_to_col_l5);
	...
	// convert to columns using im_to_col
	double* gpu_im_to_col_l6;
	cudaMalloc((void**)& gpu_im_to_col_l6, pow(output_size , 2) * pow(kernel_size , 2) * input_channels * sizeof(double));
	input_size = input_size + 2 * pad_size;
	dim3 gridDim_I6(1);
	dim3 blockDim_I6(output_size);
	input2col << < gridDim_I6, blockDim_I6 >> > (gpu_input_l6, input_size, gpu_im_to_col_l6, input_channels, output_size, stride);
	...
	weight_mult << < gridDim_W6, blockDim_W6 >> > (gpu_W6, gpu_B6, gpu_im_to_col_l6, input_channels, output_size, gpu_output_l6);
	free(W6);
	free(B6);

	cudaFree(gpu_W6);
	cudaFree(gpu_B6);
	cudaFree(gpu_im_to_col_l6);
	...
	// convert to columns using im_to_col
	double* gpu_im_to_col_l7;
	cudaMalloc((void**)& gpu_im_to_col_l7, pow(output_size,2) * pow(kernel_size,2) * input_channels * sizeof(double));
	input_size = input_size + 2 * pad_size;
	dim3 gridDim_l7(1);
	dim3 blockDim_l7(output_size);
	input2col << < gridDim_l7, blockDim_l7 >> > (gpu_input_l7, input_size, gpu_im_to_col_l7, input_channels, output_size, stride);
	...
	weight_mult << < gridDim_W7, blockDim_W7 >> > (gpu_W7, gpu_B7, gpu_im_to_col_l7, input_channels, output_size, gpu_output_l7);
	free(W7);
	free(B7);

	cudaFree(gpu_W7);
	cudaFree(gpu_B7);
	cudaFree(gpu_im_to_col_l7);
	...
	// convert to columns using im_to_col
	double* gpu_im_to_col_l8;
	cudaMalloc((void**)& gpu_im_to_col_l8, pow(output_size , 2) * pow(kernel_size, 2) * input_channels * sizeof(double));
	input_size = input_size + 2 * pad_size;
	dim3 gridDim_I8(1);
	dim3 blockDim_I8(output_size);
	input2col << < gridDim_I8, blockDim_I8 >> > (gpu_input_l8, input_size, gpu_im_to_col_l8, input_channels, output_size, stride);
	...
	weight_mult << < gridDim_W8, blockDim_W8 >> > (gpu_W8, gpu_B8, gpu_im_to_col_l8, input_channels, output_size, gpu_output_l8);
	free(W8);
	free(B8);

	cudaFree(gpu_W8);
	cudaFree(gpu_B8);
	cudaFree(gpu_im_to_col_l8);
	...
	// convert to columns using im_to_col
	double* gpu_im_to_col_l9;
	cudaMalloc((void**)& gpu_im_to_col_l9, pow(output_size , 2) * pow(kernel_size , 2) * input_channels * sizeof(double));
	input_size = input_size + 2 * pad_size;
	dim3 gridDim_I9(1);
	dim3 blockDim_I9(output_size);
	input2col << < gridDim_I9, blockDim_I9 >> > (gpu_input_l9, input_size, gpu_im_to_col_l9, input_channels, output_size, stride);
	...
	weight_mult << < gridDim_W9, blockDim_W9 >> > (gpu_W9, gpu_B9, gpu_im_to_col_l9, input_channels, output_size, gpu_output_l9);
	free(W9);
	free(B9);

	cudaFree(gpu_W9);
	cudaFree(gpu_B9);
	cudaFree(gpu_im_to_col_l9);
	...
	// convert to columns using im_to_col
	double* gpu_im_to_col_l10;
	cudaMalloc((void**)& gpu_im_to_col_l10, pow(output_size ,2) * pow(kernel_size ,2) * input_channels * sizeof(double));
	input_size = input_size + 2 * pad_size;
	dim3 gridDim_I10(1);
	dim3 blockDim_I10(output_size);
	input2col << < gridDim_I10, blockDim_I10 >> > (gpu_input_l10, input_size, gpu_im_to_col_l10, input_channels, output_size, stride);
	...
	weight_mult << < gridDim_W10, blockDim_W10 >> > (gpu_W10, gpu_B10, gpu_im_to_col_l10, input_channels, output_size, gpu_output_l10);
	free(W10);
	free(B10);

	cudaFree(gpu_W10);
	cudaFree(gpu_B10);
	cudaFree(gpu_im_to_col_l10);
	...
	// convert to columns using im_to_col
	double* gpu_im_to_col_l11;
	cudaMalloc((void**)& gpu_im_to_col_l11, pow(output_size , 2) * pow(kernel_size , 2) * input_channels * sizeof(double));
	input_size = input_size + 2 * pad_size;
	dim3 gridDim_I11(1);
	dim3 blockDim_I11(output_size);
	input2col << < gridDim_I11, blockDim_I11 >> > (gpu_input_l11, input_size, gpu_im_to_col_l11, input_channels, output_size, stride);
	...
	weight_mult << < gridDim_W11, blockDim_W11 >> > (gpu_W11, gpu_B11, gpu_im_to_col_l11, input_channels, output_size, gpu_output_l11);
	free(W11);
	free(B11);

	cudaFree(gpu_W11);
	cudaFree(gpu_B11);
	cudaFree(gpu_im_to_col_l11);
	...
	// convert to columns using im_to_col
	double* gpu_im_to_col_l12;
	cudaMalloc((void**)& gpu_im_to_col_l12, pow(output_size ,2) * pow(kernel_size , 2) * input_channels * sizeof(double));
	input_size = input_size + 2 * pad_size;
	dim3 gridDim_I12(1);
	dim3 blockDim_I12(output_size);
	input2col << < gridDim_I12, blockDim_I12 >> > (gpu_input_l12, input_size, gpu_im_to_col_l12, input_channels, output_size, stride);
	...
	weight_mult << < gridDim_W12, blockDim_W12 >> > (gpu_W12, gpu_B12, gpu_im_to_col_l12, input_channels, output_size, gpu_output_l12);
	free(W12);
	free(B12);

	cudaFree(gpu_W12);
	cudaFree(gpu_B12);
	cudaFree(gpu_im_to_col_l12);
	...
	// convert to columns using im_to_col
	double* gpu_im_to_col_l13;
	cudaMalloc((void**)& gpu_im_to_col_l13, pow(output_size , 2) * pow(kernel_size , 2) * input_channels * sizeof(double));
	input_size = input_size + 2 * pad_size;
	dim3 gridDim_I13(1);
	dim3 blockDim_I13(output_size);
	input2col << < gridDim_I13, blockDim_I13 >> > (gpu_input_l13, input_size, gpu_im_to_col_l13, input_channels, output_size, stride);
	...
	weight_mult << < gridDim_W13, blockDim_W13 >> > (gpu_W13, gpu_B13, gpu_im_to_col_l13, input_channels, output_size, gpu_output_l13);
	free(W13);
	free(B13);

	cudaFree(gpu_W13);
	cudaFree(gpu_B13);
	cudaFree(gpu_im_to_col_l13);
	...
	// convert to columns using im_to_col
	double* gpu_im_to_col_l14;
	cudaMalloc((void**)& gpu_im_to_col_l14, pow(output_size , 2) * pow(kernel_size , 2) * input_channels * sizeof(double));
	input_size = input_size + 2 * pad_size;
	dim3 gridDim_I14(1);
	dim3 blockDim_I14(output_size);
	input2col << < gridDim_I14, blockDim_I14 >> > (gpu_input_l14, input_size, gpu_im_to_col_l14, input_channels, output_size, stride);
	...
	weight_mult << < gridDim_W14, blockDim_W14 >> > (gpu_W14, gpu_B14, gpu_im_to_col_l14, input_channels, output_size, gpu_output_l14);
	free(W14);
	free(B14);

	cudaFree(gpu_W14);
	cudaFree(gpu_B14);
	cudaFree(gpu_im_to_col_l14);
	...
	// convert to columns using im_to_col
	double* gpu_im_to_col_l15;
	cudaMalloc((void**)& gpu_im_to_col_l15, pow(output_size , 2) * pow(kernel_size , 2) * input_channels * sizeof(double));
	input_size = input_size + 2 * pad_size;
	dim3 gridDim_I15(1);
	dim3 blockDim_I15(output_size);
	input2col << < gridDim_I15, blockDim_I15 >> > (gpu_input_l15, input_size, gpu_im_to_col_l15, input_channels, output_size, stride);
	...
	weight_mult << < gridDim_W15, blockDim_W15 >> > (gpu_W15, gpu_B15, gpu_im_to_col_l15, input_channels, output_size, gpu_output_l15);
	free(W15);
	free(B15);

	cudaFree(gpu_W15);
	cudaFree(gpu_B15);
	cudaFree(gpu_im_to_col_l15);
	...
	input_size = output_size;
	output_size = 7; //since we have 7 classes
	...
	cudaMemcpy(final_predictions, gpu_output_FC1, output_size * sizeof(double), cudaMemcpyDeviceToHost);
